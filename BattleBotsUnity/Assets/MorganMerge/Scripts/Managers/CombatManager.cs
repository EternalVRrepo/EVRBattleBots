...
public class CombatManager : MonoBehaviour {
	...
	void StateMovementSelection() {
		if (Input.GetButtonDown("Ability1") || Input.GetKeyDown (KeyCode.Mouse1)) {
		...
		else if (Input.GetButtonDown ("Ability3") || Input.GetKeyDown (KeyCode.Escape)) { //Skip movement phase
	...
	void StateSelectAttack() {
		...
		else if (Input.GetButtonDown ("Ability3") || Input.GetKeyDown (KeyCode.Escape)) {
			EnterStateEndOfTurn();
		}
	}
	...
	void StateEndOfTurn() {
		...
		if (CurrentParty.Count == i) {
			EnterStateEnemyTurn ();
			Debug.Log ("end of turn, enemy goes now");
		}
